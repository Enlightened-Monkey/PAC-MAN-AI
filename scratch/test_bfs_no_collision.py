...
def main():
    print("Testing BFS agent with ZERO collisions...")
    env = PacmanGridEnv(seed=42)
    obs, _ = env.reset()
    
    # Fully disable ghost movement AND collisions!
    env._state._move_ghost = lambda g: None
    
    # Mock step method of GameState to bypass collisions!
    original_step = env._state.step
    def mock_step(action):
        # We temporarily place ghosts far away before collision check, then restore
        original_positions = [g.pos for g in env._state.ghosts]
        for g in env._state.ghosts:
            g.pos = (-100, -100) # safe
        reward, done = original_step(action)
        # Restore positions
        for g, pos in zip(env._state.ghosts, original_positions):
            g.pos = pos
        return reward, done
        
    env._state.step = mock_step
